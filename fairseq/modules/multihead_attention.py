...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        dp_attn_mask: Optional[Tensor] = None,
        **kwargs,
    ) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        if dp_attn_mask is not None:
            multi_head_dp_attn_mask = torch.cat([dp_attn_mask] * self.num_heads, dim=0)
        ...
        if kwargs.get("solid_dp", False):
            src_dep_child_mask = (kwargs["src_dep_child_matrix"] == 0)
            src_dep_parent_mask = (kwargs["src_dep_parent_matrix"] == 0)
            solid_dp_mask_list = [torch.zeros_like(src_dep_child_mask) for _ in range(self.num_heads - 2)]
            solid_dp_mask_list.extend([src_dep_child_mask, src_dep_parent_mask])
            solid_dp_mask = torch.cat(solid_dp_mask_list, dim=0)
            attn_weights[solid_dp_mask] = 0.
