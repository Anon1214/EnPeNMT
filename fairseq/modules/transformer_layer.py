...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        ...
        self.dp_attn = None
        self.dp_attn_layer_norm = None

    def add_dp_attn(self, cfg):
        self.dp_attn = self.build_self_attention(self.embed_dim, cfg)
        self.dp_attn_layer_norm = LayerNorm(self.embed_dim, export=cfg.export)
    ...
    def forward(
        ...
        **kwargs
        ...
        # dp attn
        if self.dp_attn is not None:
            dp_attn_mask = kwargs["src_dep_mask_matrix"] if "src_dep_mask_matrix" in kwargs.keys() else None
            dp_attn_mask += ((dp_attn_mask == 0) * 1e-9)

            residual = x
            if self.normalize_before:
                x = self.dp_attn_layer_norm(x)
            x, attention_weights = self.dp_attn(
                query=x,
                key=x,
                value=x,
