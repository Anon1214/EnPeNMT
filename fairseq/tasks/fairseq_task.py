...
class FairseqTask(object):
    ...
    def get_batch_iterator(
        ...
        my_can_reuse_epoch_itr = (
            not getattr(self.cfg, "my_pretrain", False)
            or getattr(dataset, "my_can_reuse_epoch_itr_across_epochs", True)
        )
        logger.info(f"can_reuse_epoch_itr = {can_reuse_epoch_itr}")
        if can_reuse_epoch_itr and my_can_reuse_epoch_itr and dataset in self.dataset_to_epoch_iter:
            if hasattr(dataset, "split_name") and dataset.split_name:
                logger.info(f"reusing EpochBatchIterator of {dataset.split_name} set for epoch {epoch}")
            else:
        ...
        def make_batches(dataset, epoch):
            if hasattr(dataset, "split_name") and dataset.split_name:
                logger.info(f"creating new batches of {dataset.split_name} set for epoch {epoch}")
            else:
